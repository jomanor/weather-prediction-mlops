...
async def root():
    ...
    return {"status": "healthy", "service": "Weather API", "version": "2.0.0"}
    ...
    try:
        ...
        return CurrentWeatherResponse(
            ...
            temperature=result.get(
                "temperature", result.get("data", {}).get("main", {}).get("temp")
            ),
        ...
        async for doc in cursor:
            results.append(
                CurrentWeatherResponse(
                    city=doc["city"],
                    temperature=doc.get(
                        "temperature", doc.get("data", {}).get("main", {}).get("temp")
                    ),
                    feels_like=doc.get(
                        "feels_like",
                        doc.get("data", {}).get("main", {}).get("feels_like"),
                    ),
                    humidity=doc.get(
                        "humidity", doc.get("data", {}).get("main", {}).get("humidity")
                    ),
                    pressure=doc.get(
                        "pressure", doc.get("data", {}).get("main", {}).get("pressure")
                    ),
                    wind_speed=doc.get(
                        "wind_speed", doc.get("data", {}).get("wind", {}).get("speed")
                    ),
...
async def get_historical_weather(
    ...
    try:
        ...
        if not data:
            raise HTTPException(
                status_code=404, detail=f"No historical data found for city '{city}'"
            )

        return {"city": city, "period_hours": hours, "count": len(data), "data": data}
    except HTTPException:
        raise
    except Exception as e:
...
async def compare_cities(
    cities: str = Query(
        description="Comma-separated list of cities (e.g., 'Madrid,Barcelona,Valencia')"
    ),
...
async def get_predictions_for_city(
    ...
    try:
        cursor = (
            db.weather_predictions.find({"city": city})
            .sort("prediction_timestamp", -1)
            .limit(limit)
        )
