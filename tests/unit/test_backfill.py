...
class TestIdempotencyPattern:
    ...
    def test_upsert_true_in_source(self):
        ...
        with open(src_path) as f:
            source = f.read()
        assert "upsert=True" in source or "upsert = True" in source, (
    ...
    def test_bulk_write_or_update_one_present(self):
        ...
        with open(src_path) as f:
            source = f.read()
        assert "bulk_write" in source or "update_one" in source, (
...
class TestStructure:
    ...
    def test_open_meteo_historical_api_used(self):
        ...
        with open(src_path) as f:
            source = f.read()
        assert "archive-api.open-meteo.com" in source or "historical" in source.lower(), (
